...
describe('PlansEducationService', () => {
  ...
  describe('create', () => {
    it('should create a plans education', async () => {
      ...
      const createdPlansEducation = { id: 1, created_at: new Date(), updated_at: new Date(), deleted_at: null,  ...createPlansEducationDto };

      jest.spyOn(prisma.plansEducation, 'create').mockResolvedValue(createdPlansEducation);
  ...
  describe('findAll', () => {
    it('should return an array of plans education', async () => {
      ...
      jest.spyOn(prisma.plansEducation, 'findMany').mockResolvedValue(plansEducationList);
  ...
  describe('findOne', () => {
    it('should return a single plans education for the same student', async () => {
      ...
      const request = {
        user: { email: 'student@example.com', id_level: LEVELS.ALUNO_ESTUDANTE },
      } as any;
      const planEducation = {
            id: 1,
            professor_email: "professor@example.com",
      ...
      jest.spyOn(prisma.plansEducation, 'findUnique').mockResolvedValue(planEducation);
    ...
    it('should return a single plans education for a non-student viewing another profile', async () => {
      ...
      const request = {
        user: { email: 'professional@example.com', id_level: LEVELS.PROFISSIONAL_EDUCACAO },
      } as any;
      const planEducation = {
            id: 1,
            professor_email: "professor@example.com",
    ...
    it("should throw ForbiddenException if student views another student\'s profile", async () => {
      ...
      expect(thrownError.message).toBe('Você não tem permissão para visualizar este PEI');
    });
  });
});
